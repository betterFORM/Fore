package de.betterform.fore.xml.xforms.xpath.saxon.function.xpath;

import de.betterform.fore.xml.dom.DOMUtil;
import de.betterform.fore.xml.xforms.Container;
import de.betterform.fore.xml.xforms.xpath.saxon.function.XFormsFunction;
...
*/
public class SchemaReader extends XFormsFunction {
    private static final Log LOGGER = LogFactory.getLog(SchemaReader.class);
    /*
    ...
    private Document handleElement(XSElementDeclaration xsElementDeclaration) throws Exception {
        ...
        if (xsTypeDefinition != null) {
            if (xsTypeDefinition instanceof XSComplexTypeDefinition) {
                ...
                for (Object object : ((XSComplexTypeDecl) xsTypeDefinition).getAttributeUses()) {
        ...
        if (xsComplexTypeDefinition.getParticle() != null) {
            ...
            if (term instanceof XSModelGroup)
            {
                ...
                for (int j = 0; j < particles.getLength(); j++)
                {
                    ...
                    int min1 = particle.getMinOccurs();
                    int max1 = particle.getMaxOccurs();
                    ...
                    if (particleTerm instanceof XSElementDeclaration)
                    {
                        ...
                        if (xsElementDeclaration.getTypeDefinition() != null) {
                            XSTypeDefinition xsTypeDefinition = xsElementDeclaration.getTypeDefinition();
                            switch (xsTypeDefinition.getTypeCategory()) {
                                case XSTypeDefinition.COMPLEX_TYPE:
                                    for (Object object : ((XSComplexTypeDefinition) xsTypeDefinition).getAttributeUses()) {
                                        XSAttributeUse xsAttributeUse = (XSAttributeUse) object;
                                        childElement.setAttributeNS(xsAttributeUse.getAttrDeclaration().getNamespace(), xsAttributeUse.getAttrDeclaration().getName(), "");
                                    }
                                    break;
                            ...
                            case XSModelGroup.COMPOSITOR_SEQUENCE :
                                ...
                                for (int z = 0; z < groupParticles.getLength(); z++)
                                {
                                    ...
                                    if (groupParticleTerm instanceof XSElementDeclaration)
                                    {
                                        ...
                                        rootElement.appendChild(childElement);
                                        XSTypeDefinition xsTypeDefinition = ((XSElementDeclaration) groupParticleTerm).getTypeDefinition();
            ...
            try {
                ...
                while (it.hasNext()) {
                    ...
                    if (elementDocument != null) {
                        ...
                        return new ListIterator(de.betterform.fore.xml.xpath.impl.saxon.XPathUtil.getRootContext(elementDocument,baseURI));
                    }
                }
